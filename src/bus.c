...
static int ldbus_bus_request_name ( lua_State *L ) {
	...
	DBusError error;
	dbus_error_init ( &error );
	int result = dbus_bus_request_name ( connection , name , flags , &error );
	if ( dbus_error_is_set ( &error ) ) {
		lua_pushboolean ( L , FALSE );
		lua_pushstring ( L , error.message );
		dbus_error_free ( &error );
...
static int ldbus_bus_release_name ( lua_State *L ) {
	...
	int result = dbus_bus_release_name ( connection , name , &error );
...
static int ldbus_bus_name_has_owner ( lua_State *L ) {
	...
	int result = dbus_bus_name_has_owner ( connection , name , &error );
